...
def _reco_h1h2_for_method(
    ak4_4vec, ak8_4vec, event_idx_ak4, event_idx_ak8, arrays, method: str, cfg: CFG, Gen_match: bool = False
):
    ...
    # gen H1 is defined with pt(H1) > pt(H2); order reco H1/H2 the same way
    # so that the reconstructed Higgses are gen-matched by pt ordering.
    swap = h1.pt < h2.pt
    if Gen_match:
        return ak.where(swap, h2, h1), ak.where(swap, h1, h2)
    else:
        return h1, h2
...
def _algo_succeeded(arrays, method: str, cfg: CFG) -> np.ndarray:
    ...
    if method == "semiresolved":
        ...
        ok[ok] = ok[ok] & (ak.to_numpy(arrays[ok].ak8_msoftdrop[ak.Array(np.arange(len(arrays[ok]))),arrays[ok].semiHH_fatjet_index]) > 40)
        return ok
    if method == "merged":
        ok = np.ones(len(arrays), dtype=bool)
        ...
        ok[ok] = ok[ok] & (
            (ak.to_numpy(arrays[ok].ak8_msoftdrop[ak.Array(np.arange(len(arrays[ok]))),arrays[ok].mergedHH_H1_index]) > 40) &
            (ak.to_numpy(arrays[ok].ak8_msoftdrop[ak.Array(np.arange(len(arrays[ok]))),arrays[ok].mergedHH_H2_index]) > 40)
        )
...
def _old_reco_accuracy_for_method(arrays, method: str, cfg: CFG):
...
def permute_s4_subgroup(a):
    """
    Apply the subgroup of S4 generated by
        (12), (34), (13)(24), (14)(23)
    along axis 1.
    ...
    -------
    np.ndarray
        Array of shape (N, 4, 8), where the third axis
        indexes the 8 permutations in the subgroup.
    """
    a = np.asarray(a)
    ...
    # The subgroup generated by the specified permutations.
    permutations = [
        (0, 1, 2, 3),  # identity
        (1, 0, 2, 3),  # (12)
        (0, 1, 3, 2),  # (34)
    ...
    return np.stack([a[:, p] for p in permutations], axis=2)
...
def _reco_accuracy_for_method(arrays, method: str, cfg: CFG):
    ...
    """
    if method == "resolved":
        #Accounts for allowed permutative differences of jet assignments.
        #(N,4) array
        reco =  np.array([arrays[recoidx].to_numpy() for recoidx in cfg.resolved_reco_branches]).astype(int).T
        truth = np.array([arrays[truthidx].to_numpy() for truthidx in cfg.resolved_truth_branches]).astype(int).T
        #(N,4)->(N,4,8) 
        reco = permute_s4_subgroup(reco)
        truth = np.broadcast_to(truth[:,:,np.newaxis],reco.shape)
        correct = (reco == truth).sum(axis=1,keepdims=True).max(axis=2).flatten().astype(float)
        return correct / 4.0

    if method == "semiresolved":        
        #(N,2) array
        reco =  np.array([arrays[recoidx].to_numpy() for recoidx in cfg.semiresolved_reco_branches[1:]]).astype(int).T
        truth = np.array([arrays[truthidx].to_numpy() for truthidx in cfg.semiresolved_truth_branches[1:]]).astype(int).T
        #(N,2)->(N,2,2) 
        reco = np.stack(
            [reco[:,(0,1)],reco[:,(1,0)]],
            axis=2
        ...
        truth = np.broadcast_to(truth[:,:,np.newaxis],reco.shape)
        ak8_correct = (arrays.semiHH_fatjet_index == arrays.truerecofj_H1b1_index).to_numpy().astype(float)
        ak4_correct = (reco == truth).sum(axis=1,keepdims=True).max(axis=2).flatten().astype(float)

        correct = 0.25*ak4_correct + 0.5*ak8_correct
        return correct

    if method == "merged":
        
        #(N,2) array
        reco =  np.array([arrays[recoidx].to_numpy() for recoidx in cfg.merged_reco_branches]).astype(int).T
        truth = np.array([arrays[truthidx].to_numpy() for truthidx in cfg.merged_truth_branches]).astype(int).T
        #(N,2)->(N,2,2)
        reco = np.stack(
            [reco[:,(0,1)],reco[:,(1,0)]],
            axis=2
...
def compute_higgses(arrays, cfg: CFG,
    ...
    for j, method in enumerate(methods):
        ...
        higgses[avail, j, 8] = ak.to_numpy(reco_H1.deltaR(reco_H2)).astype(np.float32)
    return higgses


# ── target quality scores ─────────────────────────────────────────────────
def _compute_error(
    gen_H1, gen_H2, reco_H1, reco_H2, cfg, reco_accuracy,
    mH1_best_weight = CFG().mH1_best_weight,
    mH2_best_weight = CFG().mH2_best_weight,
    ptH1_best_weight = CFG().ptH1_best_weight,
    ptH2_best_weight = CFG().ptH2_best_weight,
    deltaRH1_best_weight = CFG().deltaRH1_best_weight,
    deltaRH2_best_weight = CFG().deltaRH2_best_weight,
    reco_accuracy_best_weight = CFG().reco_accuracy_best_weight,
):
    ...
    gen_mH1 = ak.to_numpy(gen_H1.mass).astype(np.float64)
    gen_mH2 = ak.to_numpy(gen_H2.mass).astype(np.float64)
    gen_ptH1 = ak.to_numpy(gen_H1.pt).astype(np.float64)
    gen_ptH2 = ak.to_numpy(gen_H2.pt).astype(np.float64)
    
    deltaRH1 = ak.to_numpy(gen_H1.deltaR(reco_H1)).astype(np.float64)
    deltaRH2 = ak.to_numpy(gen_H2.deltaR(reco_H2)).astype(np.float64)
        
    return  np.sqrt(
        np.square(mH1_best_weight * ((reco_mH1 - gen_mH1) / np.sqrt(np.square(reco_mH1) + np.square(gen_mH1)))) +
        np.square(mH2_best_weight * ((reco_mH2 - gen_mH2) / np.sqrt(np.square(reco_mH2) + np.square(gen_mH2)))) +
        np.square(ptH1_best_weight * ((reco_ptH1 - gen_ptH1) / np.sqrt(np.square(reco_ptH1) + np.square(gen_ptH1)))) +
        np.square(ptH2_best_weight * ((reco_ptH2 - gen_ptH2) / np.sqrt(np.square(reco_ptH2) + np.square(gen_ptH2)))) +
        np.square(deltaRH1_best_weight * (deltaRH1 / np.pi)) +
        np.square(deltaRH2_best_weight * (deltaRH2 / np.pi)) +
        reco_accuracy_best_weight * (1.0 - reco_accuracy)
    )
...
def _compute_targets(
    ...
    for j, method in enumerate(methods):
        ...
        # Get all necessary 4-vectors
        event_idx = ak.Array(np.arange(avail.sum()))
        reco_H1, reco_H2 = _reco_h1h2_for_method(
            ak4_4vec[avail], ak8_4vec[avail], event_idx, event_idx, arrays[avail], method, cfg,
        )
        sub_gen_H1 = gen_H1[avail] # subset arrays to available events only (avoids sentinel indexing)
        sub_gen_H2 = gen_H2[avail]
        
        reco_accuracy = _reco_accuracy_for_method(arrays[avail], method, cfg)
        # Account for mixing up H1,H2 indexing, use gen-reco pairing with lowest delta R between gen and reco higgses
        swap = (
            (reco_H1.deltaR(sub_gen_H1).to_numpy() + reco_H2.deltaR(sub_gen_H2).to_numpy()) > 
            (reco_H1.deltaR(sub_gen_H2).to_numpy() + reco_H2.deltaR(sub_gen_H1).to_numpy())
        )
        error =  np.where(
            swap,
            _compute_error(sub_gen_H2, sub_gen_H1, reco_H1, reco_H2, cfg, reco_accuracy),
            _compute_error(sub_gen_H1, sub_gen_H2, reco_H1, reco_H2, cfg, reco_accuracy)
        )
